...
class NotesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      listenable: appState,
      builder: (context, _) {
        final notes = appState.catNotes[cat.name] ?? [];
            ...
            children: [
              ...
              Padding(
                ...
                child: ElevatedButton(
                  onPressed: () => context.push('/add-note', extra: cat),
  ...
  Widget _buildNoteCard(CatNote note) {
    return Container(
      padding: const EdgeInsets.all(12),
      ...
      child: Column(
        children: [
          CircleAvatar(
            ...
            child: Icon(note.icon, color: const Color(0xFF985BEF)),
          ),
          const SizedBox(height: 10),
          Text(note.title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14), overflow: TextOverflow.ellipsis),
          Text(note.date, style: const TextStyle(color: Colors.grey, fontSize: 10)),
          const Spacer(),
          Container(
