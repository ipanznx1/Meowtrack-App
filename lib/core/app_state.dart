...
class AppStateController extends ChangeNotifier {
  ...
  Future<List<String>> getAvailableAiModels() async {
    String key = "";
    try {
      // Cuba dapatkan key dari pelbagai punca
      key = (await const FlutterSecureStorage().read(key: 'GEMINI_API_KEY')) ?? "";
      if (key.isEmpty) {
        final remoteConfig = FirebaseRemoteConfig.instance;
        key = remoteConfig.getString("gemini_api_key");
      }
    } catch (_) {}

    if (key.isEmpty) return ["Ralat: Tiada API Key dijumpai."];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return models.map((m) => m['name'].toString().replaceAll('models/', '')).toList();
      } else {
        return ["Ralat Server: ${response.statusCode}", "Mesej: ${response.body}"];
      }
    } catch (e) {
